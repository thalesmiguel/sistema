class FazendasController < ApplicationController
  ...
  def create
    ...
    if @fazenda.save
      renderiza_crud_js(@fazenda, 'Endereço criado com sucesso.')
    else
  ...
  def update
    if @fazenda.update(fazenda_params)
      renderiza_crud_js(@fazenda, 'Endereço alterado com sucesso.')
    else
      renderiza_crud_js(@fazenda)
    end
  end
  ...
  def destroy
    @fazenda.destroy
    renderiza_crud_js(@fazenda, 'Endereço excluído com sucesso.')
  end
