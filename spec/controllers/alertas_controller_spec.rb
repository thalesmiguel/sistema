...
RSpec.describe AlertasController, type: :controller do
  ...
  describe "POST create" do
    ...
    context 'dados válidos' do
      let(:dados_validos) { FactoryGirl.attributes_for(:alerta, cliente_id: cliente) }

      it 'renderiza novo Alerta' do
        post :create, xhr: true, params: { cliente_id: cliente.id, alerta: dados_validos }
    ...
    context 'dados inválidos' do
      let(:dados_invalidos) { FactoryGirl.attributes_for(:alerta, cliente_id: cliente, descricao: '') }

      it 'renderiza mensagem de erro' do
        post :create, xhr: true, params: { cliente_id: cliente.id, alerta: dados_invalidos }
        expect(response).to render_template("ajax/application/crud.js.erb")
      end
  ...
  describe "GET edit" do
    ...
    it 'renderiza modal :edit' do
      get :edit, xhr: true, params: { cliente_id: cliente.id, id: alerta }
      expect(response).to render_template('ajax/application/mostra_modal.js.erb')
    end
  ...
  describe "PUT update" do
    ...
    context 'dados válidos' do
      let(:dados_validos) { FactoryGirl.attributes_for(:alerta, cliente_id: cliente, descricao: "Descrição completa") }

      it 'renderiza Alerta alterado' do
        put :update, xhr: true, params: { cliente_id: cliente.id, id: alerta, alerta: dados_validos }
        expect(response).to render_template("ajax/application/crud.js.erb")
      end
    ...
    context 'dados inválidos' do
      ...
      it 'renderiza mensagem de erro' do
        put :update, xhr: true, params: { cliente_id: cliente.id, id: alerta, alerta: dados_invalidos }
        expect(response).to render_template("ajax/application/crud.js.erb")
      end
  ...
  describe "DELETE destroy" do
    ...
    it 'deleta Alerta da tabela' do
      delete :destroy, xhr: true, params: { cliente_id: cliente.id, id: alerta }
      expect(response).to render_template("ajax/application/crud.js.erb")
    end
